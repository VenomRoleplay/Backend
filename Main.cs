...
namespace GVRP
{
    ...
    "Benutze I um auf deinen Spind zuzugreifen");
        ...
        public void OnPlayerDeath(Client player, Client killer, uint weapon)
        {
            ...
            if(!killer.Exists) { return; }

            if (!killer.IsInVehicle)
            {
                Discord.SendMessage($"{killer.Name} hat {player.Name} getötet. ({killer.CurrentWeapon})", "KILL-LOG");
                return;
            }
            else
            {
                if(killer.CurrentWeapon == WeaponHash.Unarmed)
                {
                    Discord.SendMessage($"{killer.Name} hat {player.Name} überfahren. ({killer.CurrentWeapon})", "DRIVE-BY KILL-LOG");
                    int speed = killer.GetSpeed();
                    GVRP.Module.Players.Players.Instance.SendMessageToAuthorizedUsers("log", $"{killer.Name} hat {player.Name} überfahren. ({speed}) km/h)", 3500);
                    return;
                }
                else
                {
                    Discord.SendMessage($"{killer.Name} hat {player.Name} im Auto getötet. ({killer.CurrentWeapon})", "DRIVE-BY KILL-LOG");
                    return;
                }
            }

            Discord.SendMessage($"{player.Name} ist gestorben ({killer.CurrentWeapon})");
        ...
        public async void onPlayerEnterVehicle(Client player, Vehicle vehicle, sbyte seat)
        {
